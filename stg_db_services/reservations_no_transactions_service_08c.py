...
async def step_1_count_total_no_transactions_reservations(account_id: str, location_id: int, engine):
    """
...
async def process_reservations_no_transactions(account_id: str, location_id: int, engine):
    """
    Main function to process no-transactions reservations data
    5-step process: count records, validate dependencies, insert data
    Dependencies: customers, class_sessions tables
    """
    logger.info(f"[process_reservations_no_transactions] Starting processing for account_id={account_id}, location_id={location_id}")
    
    try:
        # Step 1: Count total no-transactions reservations
        try:
            total_records_result = await step_1_count_total_no_transactions_reservations(account_id, location_id, engine)
