...
async def step_0_drop_duplicates_in_staging(account_id: str, location_id: int, engine):
    """
    Step 0: Drop duplicate Other transaction type records in staging table (mt_order_lines_details_dlk) to clean data
    """
    logger.info(f"[STEP 0] Dropping duplicate Other transaction records in mt_order_lines_details_dlk staging table")
...
async def process_order_lines_other(account_id: str, location_id: int, engine):
    """
    ...
    Dependencies: orders table only
    """
    logger.info(f"[process_order_lines_other] Starting processing for account_id={account_id}, location_id={location_id}")
