...
async def step_0_drop_duplicates_in_staging(account_id: str, location_id: int, engine):
    """
    Step 0: Drop duplicate records in staging table (mt_memb        # Step 5: Enhanced validation before insertion
        ...
        if ready_to_insert != expected_ready:
            ...
            raise Exception(error_msg)ances_details_dlk) to clean data
    """
    logger.info(f"[STEP 0] Dropping duplicate records in mt_membership_instances_details_dlk staging table")
        ...
        with engine.begin() as conn:
            ...
            count_result = conn.execute(text("""
                SELECT 
                    ...
                    COUNT(DISTINCT membership_instances_id) as unique_instances
            ...
            unique_instances = stats[1]
            duplicates_found = total_before - unique_instances
            
            if duplicates_found > 0:
                logger.info(f"[STEP 0] Found {duplicates_found} duplicate records to remove ({total_before} total, {unique_instances} unique)")
                
                # Delete duplicates, keeping only one record per membership_instances_id
                result = conn.execute(text("""
                    DELETE FROM mt_membership_instances_details_dlk 
                    WHERE ctid NOT IN (
                        SELECT DISTINCT ON (membership_instances_id) ctid
                        FROM mt_membership_instances_details_dlk
                        WHERE account_id = :account_id
                          AND location = :location_id
                        ORDER BY membership_instances_id, ctid
                    )
