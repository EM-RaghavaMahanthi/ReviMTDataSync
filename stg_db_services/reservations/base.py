...
async def step_3_recalculate_first_timer(account_id: str, location_id: int, engine):
    """
    ...
    Logic:
      ...
      - Multiple reservations            → earliest 'pending' or 'check in' by class start_datetime
                                           fallback to earliest overall if none match
    """
    logger.info(f"[STEP 3] Recalculating first_timer on main table for account_id={account_id}, location_id={location_id}")

    PARAMS = {"account_id": account_id, "location_id": location_id}
...
async def process_reservations(account_id: str, location_id: int, engine):
    ...
    """
    logger.info(f"[process_reservations] Starting for account_id={account_id}, location_id={location_id}")

    try:
        # Step 0: dedup staging
        step_0_result      = drop_staging_duplicates(engine, "mt_reservations_details_dlk", "reservations_id", account_id, location_id)
        ...
        with engine.connect() as conn:
            total_staging = conn.execute(text("""
                SELECT COUNT(*) FROM mt_reservations_details_dlk
                WHERE account_id = :account_id AND location = :location_id
            """), {"account_id": account_id, "location_id": str(location_id)}).scalar()
        logger.info(f"[process_reservations] Staging records after dedup: {total_staging}")

        # Step 2: insert staging → main
        step_2_result          = await step_2_orchestrate_reservations_processing(account_id, location_id, engine)
        ...
        step_3_result       = await step_3_recalculate_first_timer(account_id, location_id, engine)
