"""
Reservations Membership Service - Step 8b of ETL Pipeline
Processes membership_transactions reservations from staging to main table
Dependencies: customers, class_sessions, membership_transactions tables
"""
...
async def step_1_count_total_membership_reservations(account_id: str, location_id: int, engine):
    """
    Step 1: Count total membership_transactions reservation records in staging (no duplicates needed - already done in service 08)
    """
    logger.info(f"[STEP 1] Counting total membership_transactions reservation records in staging")
        ...
        with engine.begin() as conn:
            result = conn.execute(text("""
                ...
                WHERE account_id = :account_id
                  ...
                  AND membership_transactions_id IS NOT NULL
            """), {"account_id": account_id, "location_id": str(location_id)})
            total_count = result.fetchone()[0]
        
        logger.info(f"[STEP 1] SUCCESS: Total membership_transactions reservation records: {total_count}")
        return {"total_membership_reservations": total_count}
    except Exception as e:
        logger.error(f"[STEP 1] ERROR: Failed to count total membership_transactions reservation records: {e}")
        raise

async def step_2_count_existing_in_main_table(account_id: str, location_id: int, engine):
    """
    Step 2: Count membership_transactions reservation records already existing in main table
    Uses same JOIN conditions as insert logic to ensure accurate counting
    """
    logger.info(f"[STEP 2] Counting membership_transactions reservation records already in main table")
    ...
    try:
        ...
        logger.info(f"[STEP 2] SUCCESS: Membership_transactions reservation records already exist in main table: {existing_count}")
        return existing_count
    except Exception as e:
        logger.error(f"[STEP 2] ERROR: Failed to count existing membership_transactions reservation records: {e}")
        raise

async def step_3_count_records_with_missing_dependencies(account_id: str, location_id: int, engine):
    """
    Step 3: Count membership_transactions reservation records with missing dependencies (invalid for insertion)
    Dependencies: customers, class_sessions, membership_transactions tables
    """
    logger.info(f"[STEP 3] Counting membership_transactions reservation records with missing dependencies")
    ...
    try:
        with engine.begin() as conn:
            ...
            # Count records with missing membership_transactions
            missing_membership_transactions_result = conn.execute(text("""
                SELECT COUNT(*) as count
                FROM mt_reservations_details_dlk stg
                WHERE stg.account_id = :account_id
                  AND stg.location = :location_id
            ...
            """), {"account_id": account_id, "location_id": str(location_id)})
            missing_membership_transactions_count = missing_membership_transactions_result.fetchone()[0]
        ...
        if total_invalid_dependencies > 0:
            logger.warning(f"[STEP 3] Found {total_invalid_dependencies} membership_transactions reservation records with missing dependencies")
            ...
            logger.warning(f"[STEP 3] Missing membership_transactions: {missing_membership_transactions_count}")
        
        logger.info(f"[STEP 3] SUCCESS: Membership_transactions reservation records with missing dependencies: {total_invalid_dependencies}")
...
async def step_5_insert_new_records(account_id: str, location_id: str, engine):
    ...
    try:
        with engine.begin() as conn:
            result = conn.execute(text("""
                ...
                SELECT 
                  ...
                  stg.reservation_type as prev_reservation_type, stg.transactions_type
                FROM mt_reservations_details_dlk stg
                INNER JOIN customers c
        ...
        logger.error(f"[STEP 5] ERROR: Failed to insert membership_transactions reservation records: {e}")
        raise

async def process_reservations_membership(account_id: str, location_id: int, engine):
    """
    Main function to process membership_transactions reservations data
    5-step process: count records, validate dependencies, insert data
    Dependencies: customers, class_sessions, membership_transactions tables
    """
    logger.info(f"[process_reservations_membership] Starting processing for account_id={account_id}, location_id={location_id}")
    
    try:
        # Step 1: Count total membership reservations
        try:
            total_records_result = await step_1_count_total_membership_reservations(account_id, location_id, engine)
            total_membership_reservations = total_records_result["total_membership_reservations"]
        except Exception as e:
            logger.error(f"[process_reservations_membership] ERROR: Step 1 failed: {e}")
            raise
        ...
        try:
            ...
            logger.error(f"[process_reservations_membership] ERROR: Step 2 failed: {e}")
            raise
        ...
        try:
            ...
            logger.error(f"[process_reservations_membership] ERROR: Step 3 failed: {e}")
            raise
        
        # Calculate expected records for insertion
        expected_ready = total_membership_reservations - already_exist_in_main - invalid_dependency_records
        ...
        if expected_ready > 0:
            try:
                ...
                logger.error(f"[process_reservations_membership] ERROR: Step 4 failed: {e}")
                raise
            
            # Validate that insert_ready_count matches expected_ready
            if insert_ready_count != expected_ready:
                logger.error(f"[process_reservations_membership] ERROR: Validation failed - expected {expected_ready} but found {insert_ready_count} ready for insertion")
                raise Exception(f"Insert ready count mismatch: expected {expected_ready}, actual {insert_ready_count}")
            
            logger.info(f"[process_reservations_membership] SUCCESS: Validation passed - {insert_ready_count} records ready for insertion as expected")
            
            # Step 5: Insert new records
            try:
                inserted_records = await step_5_insert_new_records(account_id, location_id, engine)
            except Exception as e:
                logger.error(f"[process_reservations_membership] ERROR: Step 5 failed: {e}")
                raise
            
            # Final validation that insertion count matches expected
            if inserted_records == expected_ready:
                logger.info(f"[process_reservations_membership] SUCCESS: Final validation passed - inserted {inserted_records} records as expected")
            else:
                logger.error(f"[process_reservations_membership] ERROR: Final validation failed - expected {expected_ready} but inserted {inserted_records}")
                raise Exception(f"Final insertion count mismatch: expected {expected_ready}, actual {inserted_records}")
        else:
            logger.warning(f"[process_reservations_membership] WARNING: No records to insert (expected_ready: {expected_ready})")
            insert_ready_count = 0
            inserted_records = 0
        
        # Calculate missing percentage: (total - already_existing - inserted) / total * 100
        total_not_inserted = total_membership_reservations - already_exist_in_main - inserted_records
        missing_percentage = round((total_not_inserted / total_membership_reservations * 100), 2) if total_membership_reservations > 0 else 0.0
        
        # Summary
        logger.info(f"[process_reservations_membership] PROCESS COMPLETE:")
        logger.info(f"  - Total membership_transactions reservations: {total_membership_reservations}")
        ...
        logger.error(f"[process_reservations_membership] FATAL ERROR: {e}")
        raise