...
async def step_4_count_membership_transactions_with_missing_customers(account_id: str, location_id: int, engine):
    """
    Step 3: Count membership_transactions records with missing customer dependencies (invalid for insertion)
    Also counts the number of distinct missing customers for insights
    """
    logger.info(f"[STEP 3] Counting membership_transactions records with missing customer dependencies")
    
    try:
        with engine.begin() as conn:
            # Count membership_transactions records where customer_id is NULL or doesn't exist in customers table
            result = conn.execute(text("""
                SELECT COUNT(*) as count
                FROM mt_membership_transactions_details_dlk stg
                WHERE stg.account_id = :account_id
        ...
        if missing_customer_transactions > 0:
            logger.warning(f"[STEP 3] Found {missing_customer_transactions} membership_transactions records with missing customers")
            logger.warning(f"[STEP 3] Found {distinct_missing_customers} distinct missing customers")
            logger.warning(f"[STEP 3] Found {null_customer_transactions} membership_transactions records with NULL customer_id")
        ...
        logger.info(f"[STEP 3] SUCCESS: Membership_transactions records with missing customers: {missing_customer_transactions}")
        logger.info(f"[STEP 3] SUCCESS: Distinct missing customers: {distinct_missing_customers}")
        logger.info(f"[STEP 3] SUCCESS: Membership_transactions records with NULL customer_id: {null_customer_transactions}")
        return missing_customer_transactions
    except Exception as e:
        logger.error(f"[STEP 3] ERROR: Failed to count membership_transactions with missing customer dependencies: {e}")
        raise
        ...
        with engine.begin() as conn:
            result = conn.execute(text("""
                ...
                SELECT  
                   ...
                   membership_instances_id, mt.customer_id, location, now() created_at, 1 as created_by, now() updated_at, mt.updated_by, 
                   mt.deleted_at, mt.deleted_by, mt.payment_interval_end_date, mt.account_id, membership_instances_ref_id
                FROM mt_membership_transactions_details_dlk mt
                INNER JOIN customers c 
                ON mt.customer_id = c.customer_id  
                AND mt.account_id = c.account_id
...
async def process_membership_transactions_orders(account_id: str, location_id: int, engine):
    ...
    try:
        ...
        try:
            invalid_customer_records = await step_4_count_membership_transactions_with_missing_customers(account_id, location_id, engine)
        except Exception as e:
