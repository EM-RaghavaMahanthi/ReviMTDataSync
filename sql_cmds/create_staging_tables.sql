-- Staging tables for stage 2 (S3 -> staging), executed wholesale by
-- db_services/main_bulk_insert.etl_all_tables.
--
-- Only the three notes/tags tables are created. RDS onboarding no longer runs for the CRM
-- tables, so their staging is dead weight: TABLE_S3_CONFIG stopped loading them, and the
-- stale update that was their only other reader is gone too. Creating them would leave
-- eight empty tables that look like they mean something.
--
-- The eight are COMMENTED, not deleted, and their column lists are still accurate — restore
-- one by uncommenting its DROP and its CREATE together, alongside the matching entry in
-- TABLE_S3_CONFIG.
