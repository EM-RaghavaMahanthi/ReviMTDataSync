...
async def step_0_drop_duplicates_in_staging(account_id: str, engine):
    """
    Step 0: Drop duplicate MembershipTransaction records in staging table (stg_order_lines_bulk) to clean data
    """
    logger.info(f"[STEP 0] Dropping duplicate MembershipTransaction records in stg_order_lines_bulk staging table")
...
async def step_3_count_records_already_exist_in_main_table(account_id: str, engine):
    ...
    return await step_2_count_existing_in_main_table(account_id, engine)
...
async def step_7_insert_records(account_id: str, engine):
    """
...
async def process_order_lines_membership(account_id: str, engine):
    """
    ...
    """
    logger.info(f"[process_order_lines_membership] Starting processing for account_id={account_id}")
    ...
    try:
        ...
        if expected_ready > 0:
            ...
            try:
                inserted_records = await step_7_insert_records(account_id, engine)
            except Exception as e:
