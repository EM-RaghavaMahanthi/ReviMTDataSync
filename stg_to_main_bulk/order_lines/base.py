...
try:
    ...
    try:
        ...
        from stg_to_main_bulk.order_lines.credit import process_order_lines_credit
        from stg_to_main_bulk.order_lines.membership import process_order_lines_membership
        from stg_to_main_bulk.order_lines.other import process_order_lines_other

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

async def process_order_lines(account_id: str, engine):
    """
    Orchestrate sequential processing of all order_lines transaction types
    Execution Order: 7a (Credit) → 7b (Membership) → 7c (Other)
    """
    logger.info(f"[process_order_lines] Starting orchestrated processing for account_id={account_id}")
    ...
    try:
        ...
        try:
            result_7a = await process_order_lines_credit(account_id, engine)
        ...
        try:
            result_7b = await process_order_lines_membership(account_id, engine)
        ...
        try:
            result_7c = await process_order_lines_other(account_id, engine)
