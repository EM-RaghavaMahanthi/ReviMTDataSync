...
async def step_1_count_staging_total(account_id: str, engine):
    ...
    """
    logger.info(f"[STEP 1] Counting total entries in stg_membership_instances_bulk")
    
    try:
        with engine.begin() as conn:
            result = conn.execute(text("SELECT COUNT(*) as count FROM public.stg_membership_instances_bulk"))
...
async def step_3_count_records_already_exist_in_main_table(account_id: str, engine):
    ...
    try:
        with engine.begin() as conn:
            result = conn.execute(text("""
                SELECT COUNT(*) as count
                FROM stg_membership_instances_bulk stg
    ...
    try:
        insert_sql = text("""
            ...
            SELECT
              ...
              updated_by, deleted_at, deleted_by, account_id
            FROM stg_membership_instances_bulk
            WHERE account_id = :account_id
...
async def process_membership_instances(account_id: str, engine):
    """
    ...
    """
    logger.info(f"[process_membership_instances] Starting membership_instances processing for account_id={account_id}")
    ...
    try:
        ...
        try:
            dup_stats = drop_staging_duplicates(engine, "stg_membership_instances_bulk", "membership_instances_id", account_id)
        ...
        # Summary logging
        logger.info(f"[process_membership_instances] SUMMARY for account_id={account_id}:")
        ...
        logger.info(f"[process_membership_instances] SUCCESS: Completed membership_instances processing for account_id={account_id}")
        ...
        logger.error(f"[process_membership_instances] ERROR: Failed processing for account_id={account_id}: {e}")
