...
async def step_1_count_total_membership_reservations(account_id: str, engine):
    """
...
async def process_reservations_membership(account_id: str, engine):
    """
    ...
    Dependencies: customers, class_sessions, membership_transactions tables
    """
    logger.info(f"[process_reservations_membership] Starting processing for account_id={account_id}")
    
    try:
        # Step 1: Count total membership reservations
        try:
            total_records_result = await step_1_count_total_membership_reservations(account_id, engine)
