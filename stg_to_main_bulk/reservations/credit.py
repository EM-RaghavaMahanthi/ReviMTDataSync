...
async def step_1_count_total_credit_reservations(account_id: str, engine):
    """
...
async def process_reservations_credit(account_id: str, engine):
    """
    ...
    Dependencies: customers, class_sessions, credit_transactions tables
    """
    logger.info(f"[process_reservations_credit] Starting processing for account_id={account_id}")
    
    try:
        # Step 1: Count total credit reservations
        try:
            total_records_result = await step_1_count_total_credit_reservations(account_id, engine)
