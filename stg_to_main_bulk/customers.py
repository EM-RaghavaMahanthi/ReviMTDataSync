...
async def step_1_count_staging_total(account_id: str, engine):
    ...
    """
    logger.info(f"[STEP 1] Counting total entries in stg_customers_bulk")
    
    try:
        with engine.connect() as conn:
            result = conn.execute(text("SELECT COUNT(*) as count FROM public.stg_customers_bulk"))
...
async def step_2_count_null_mandatory_names(account_id: str, engine):
    """
    ...
    try:
        with engine.begin() as conn:
            result = conn.execute(text("""
                ...
                FROM public.stg_customers_bulk 
...
async def step_3_count_existing_in_main_table(account_id: str, engine):
    """
...
async def step_4_count_records_already_exist(account_id: str, engine):
    """
...
async def step_5_count_duplicates_in_staging(account_id: str, engine):
    """
        ...
        insert_sql = text("""
            ...
            WHERE account_id = :account_id
              ...
              AND customer_id NOT IN (
                SELECT customer_id FROM customers WHERE account_id = :account_id
              )
        """)
...
async def process_customers(account_id: str, engine):
    """
    Process customers from staging to final table.
    Executes the transformation in 7 detailed steps with comprehensive logging.
    """
    logger.info(f"[process_customers] Starting customer processing for account_id={account_id}")
    ...
    try:
        ...
        try:
            step_0_result = drop_staging_duplicates(engine, "stg_customers_bulk", "customer_id", account_id)
        ...
        try:
            null_mandatory_names = await step_2_count_null_mandatory_names(account_id, engine)
        except Exception as e:
        ...
        try:
            existing_in_main = await step_3_count_existing_in_main_table(account_id, engine)
        except Exception as e:
        ...
        try:
            already_existing = await step_4_count_records_already_exist(account_id, engine)
        except Exception as e:
        ...
        try:
            duplicate_in_stg = await step_5_count_duplicates_in_staging(account_id, engine)
        except Exception as e:
        ...
        try:
            ready_to_insert = await step_6_count_records_to_insert(account_id, engine)
        except Exception as e:
        ...
        if ready_to_insert == 0:
            ...
            try:
                actual_inserted = await step_7_insert_new_records(account_id, engine)
            except Exception as e:
                logger.error(f"[process_customers] ERROR: Step 7 failed: {e}")
                raise
        
        # Summary logging
        logger.info(f"[process_customers] SUMMARY for account_id={account_id}:")
        ...
        logger.info(f"[process_customers] SUCCESS: Completed customer processing for account_id={account_id}")
        ...
        logger.error(f"[process_customers] ERROR: Failed processing for account_id={account_id}: {e}")
