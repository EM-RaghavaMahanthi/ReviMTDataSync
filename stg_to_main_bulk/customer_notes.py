...
async def step_1_count_staging_total(account_id: str, engine):
    ...
    logger.info(f"[STEP 1] Counting total entries in stg_user_notes_bulk")
    try:
        with engine.begin() as conn:
            result = conn.execute(text("""
                SELECT COUNT(*) as count FROM public.stg_user_notes_bulk
...
async def process_customer_notes(account_id: str, engine):
    """
    Process customer_notes from staging to final table.
    Not location-scoped — user_notes has no location filter at the CRM layer, and in the
    bulk pipeline no step is location-scoped at all.
    """
    ...
    try:
        dup_stats = drop_staging_duplicates(engine, "stg_user_notes_bulk", "note_id", account_id)
