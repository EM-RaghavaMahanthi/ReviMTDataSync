...
async def step_1_count_staging_total(account_id: str, engine):
    ...
    logger.info(f"[STEP 1] Counting tag rows in stg_user_tags_bulk")
    try:
        with engine.begin() as conn:
            result = conn.execute(text("""
                SELECT COUNT(*) as count FROM stg_user_tags_bulk
        ...
        with engine.begin() as conn:
            result = conn.execute(text(f"""
                ...
                WHERE d.tenant_name IN (
                  SELECT DISTINCT tenant_name FROM stg_user_tags_bulk
                  WHERE account_id = :account_id
                )
            """), {"account_id": account_id})
...
async def process_customer_tags_default(account_id: str, engine):
    """Process all tags from stg_user_tags_bulk into customer_tags_default."""
    ...
    try:
        ...
        dup_stats = drop_staging_duplicates(engine, "stg_user_tags_bulk", "tag_id", account_id)
