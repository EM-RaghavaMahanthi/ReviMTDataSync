...
async def step_1_count_staging_total(account_id: str, engine):
    ...
    """
    logger.info(f"[STEP 1] Counting total entries in stg_credit_transactions_bulk")
    
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                SELECT COUNT(*) as count
                FROM stg_credit_transactions_bulk
...
async def step_1b_count_unnecessary_records(account_id: str, engine):
    """
    Step 1b: always 0 in the bulk pipeline — kept because process_credit_transactions's
...
async def step_3_count_transactions_with_missing_customers(account_id: str, engine):
    """
        ...
        with engine.connect() as conn:
            result = conn.execute(text("""
                WITH missing_customers AS (
                    ...
                    FROM stg_credit_transactions_bulk stg
...
async def step_4_count_records_already_exist_in_main_table(account_id: str, engine):
    """
...
async def step_6_insert_new_records(account_id: str, engine):
    ...
    try:
        insert_sql = text("""
            ...
            FROM stg_credit_transactions_bulk mt
...
async def process_credit_transactions(account_id: str, engine):
    """
    ...
    """
    logger.info(f"[process_credit_transactions] Starting credit_transactions processing for account_id={account_id}")
    ...
    try:
        ...
        try:
            dup_stats = drop_staging_duplicates(engine, "stg_credit_transactions_bulk", "credit_transactions_id", account_id)
        ...
        try:
            missing_customer_transactions = await step_3_count_transactions_with_missing_customers(account_id, engine)
        except Exception as e:
        ...
        try:
            already_exist_in_main = await step_4_count_records_already_exist_in_main_table(account_id, engine)
        except Exception as e:
        ...
        # Summary logging
        logger.info(f"[process_credit_transactions] SUMMARY for account_id={account_id}:")
        ...
        logger.info(f"[process_credit_transactions] SUCCESS: Completed credit_transactions processing for account_id={account_id}")
        ...
        logger.error(f"[process_credit_transactions] ERROR: Failed processing for account_id={account_id}: {e}")
        ...
        logger.error(f"[process_credit_transactions]   Unnecessary records (always 0 in bulk): {unnecessary_records}")
