...
def create_teams_client_from_config() -> Optional[TeamsClient]:
    """
    Create TeamsClient from whichever settings module is available
    (core.config for Stage 1/2, core.stg_db_config for Stage 3).
    Returns None if TEAMS_ENABLED is False or webhook URL is not set.
    """
    try:
        try:
            from core.config import settings
        except Exception:
            from core.stg_db_config import settings  # type: ignore

        if not settings.TEAMS_ENABLED:
            logger.debug("TEAMS_ENABLED=False — notifications disabled")
            return None

        if not settings.TEAMS_WEBHOOK_URL:
            logger.warning("TEAMS_ENABLED=True but TEAMS_WEBHOOK_URL is not set")
            return None

        return TeamsClient(
            webhook_url=settings.TEAMS_WEBHOOK_URL,
            timeout=settings.TEAMS_TIMEOUT_SECONDS,
            max_retries=settings.TEAMS_MAX_RETRIES,
        )

    except Exception as e:
        logger.error(f"Failed to create TeamsClient from config: {e}")
        return None
