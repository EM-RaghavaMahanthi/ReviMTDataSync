...
async def fetch_membership_transactions_for_user(user_id: str, account_id: str, api_base_url: str, location_id: int):
    """
    Fetch all membership transactions for a user without saving to S3.
    ...
    logger.info(f"[FETCH] User {user_id} (account: {account_id}) completed. Fetched {len(all_transactions)} membership transactions.")
...
async def process_membership_transactions_batch(user_ids: list, batch_id: int, account_id: str, api_base_url: str, location_id: int):
    ...
    logger.info(f"[BATCH {batch_id}] Processing {len(user_ids)} users for membership transactions (account: {account_id})")
    ...
    async def guarded_fetch_user(user_id):
        async with semaphore:
            try:
                ...
                return await fetch_membership_transactions_for_user(user_id, account_id, api_base_url, location_id)
            except Exception as e:
    ...
    logger.info(f"[BATCH {batch_id}] Aggregated {len(all_data)} membership transactions from {len(user_ids)} users (account: {account_id})")
