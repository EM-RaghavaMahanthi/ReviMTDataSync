...
import asyncio
from services.membership_transactions_service import fetch_membership_transactions_page
...
async def write_failed_entries(failed_entries, batch_id, account_id, location_id):
    ...
    dlq_file = f"data/failed_membership_transactions_batch_{batch_id}_account_{account_id}_location_{location_id}.json"
        ...
        async with semaphore:
            ...
            for failure in failures:
                ...
                try:
                    ...
                    transactions, _ = await fetch_membership_transactions_page(
                        user_id, failure["page"], account_id, api_base_url, location_id
                    )
                    user_recovered.extend(transactions)
                    logger.info(f"[DLQ RETRY] Batch {batch_id}: User {user_id} page {failure['page']} recovered - {len(transactions)} transactions")
...
async def fetch_membership_transactions_for_user(user_id: str, account_id: str, api_base_url: str, location_id: int):
    """
    Fetch all membership transactions for a user with failure tracking.
    Returns all data, expected count, and failed pages for aggregation.
    """
    logger.info(f"[FETCH] Starting membership transactions fetch for user {user_id} (account: {account_id})")
    
    failed_pages = []  # In-memory failure collection
    
    try:
        # Get first page to know total pages
        first_transactions, first_resp = await fetch_membership_transactions_page(user_id, 1, account_id, api_base_url, location_id)
    except Exception as e:
    ...
    logger.info(f"[FETCH] User {user_id} (account: {account_id}): Expected {total_records} membership transactions across {total_pages} pages.")
    ...
    if total_pages > 1:
        ...
        async def fetch_page(page):
            async with semaphore:
                try:
                    transactions, _ = await fetch_membership_transactions_page(user_id, page, account_id, api_base_url, location_id)
    ...
    logger.info(f"[FETCH] User {user_id} (account: {account_id}) completed. Fetched {len(all_transactions)} membership transactions, {len(failed_pages)} page failures.")
    return all_transactions, total_records, failed_pages

async def process_membership_transactions_batch(user_ids: list, batch_id: int, account_id: str, api_base_url: str, location_id: int):
    """
    Process multiple users and save aggregated data with batch_id, including DLQ handling.
    """
    start_time = time.time()
    logger.info(f"[BATCH {batch_id}] Processing {len(user_ids)} users for membership transactions (account: {account_id}, location: {location_id})")
    ...
    async def guarded_fetch_user(user_id):
        async with semaphore:
            try:
                ...
                user_data, expected, user_failures = await fetch_membership_transactions_for_user(user_id, account_id, api_base_url, location_id)
                # Collect failures from this user
                all_batch_failures.extend(user_failures)
                return user_data, expected
            except Exception as e:
                logger.error(f"[BATCH {batch_id}] User {user_id} (account: {account_id}, location: {location_id}) completely failed: {e}")
    ...
    logger.info(f"[BATCH {batch_id}] Initial aggregation: {len(all_data)} membership transactions from {len(user_ids)} users (account: {account_id})")
    ...
    if all_batch_failures:
        ...
        if recovered_data:
            ...
            logger.info(f"[DLQ RECOVERY] Batch {batch_id}: {len(recovered_data)} transactions recovered from retry")
    
    # Save in chunks with batch_id (keeping original S3 naming)
    ...
    for i in range(0, len(all_data), parquet_batch_size):
        ...
        await write_parquet_to_s3(
            batch,
            str(batch_id),  # Original naming convention
            file_num,
            account_id,
            s3_prefix=prefix,
        ...
        logger.info(f"[BATCH {batch_id}] Saved file {file_num} with {len(batch)} records (account: {account_id}, location: {location_id})")
        file_num += 1
    ...
    logger.info(f"[BATCH {batch_id}] Completed in {elapsed:.2f} seconds. Total processed: {total_processed}, Expected: {total_expected} (account: {account_id}, location: {location_id})")
    ...
    if final_failed_count > 0:
        ...
        raise Exception(f"Membership transactions batch {batch_id} failed: {final_failed_count} user pages could not be processed after retry")
