...
async def write_failed_entries(failed_entries, batch_id, account_id, location_id):
    ...
    dlq_file = f"data/failed_credit_transactions_batch_{batch_id}_account_{account_id}_location_{location_id}.json"
        ...
        async with semaphore:
            ...
            for failure in failures:
                ...
                try:
                    ...
                    transactions, _ = await fetch_credit_transactions_page(
...
async def fetch_credit_transactions_for_user(user_id: str, account_id: str, api_base_url: str, location_id: int):
    """
    Fetch all credit transactions for a user with failure tracking.
    ...
    logger.info(f"[FETCH] User {user_id} (account: {account_id}) completed. Fetched {len(all_transactions)} credit transactions, {len(failed_pages)} page failures.")
...
async def process_credit_transactions_batch(user_ids: list, batch_id: int, account_id: str, api_base_url: str, location_id: int):
    ...
    logger.info(f"[BATCH {batch_id}] Processing {len(user_ids)} users for credit transactions (account: {account_id}, location: {location_id})")
    ...
    async def guarded_fetch_user(user_id):
        async with semaphore:
            try:
                ...
                user_data, expected, user_failures = await fetch_credit_transactions_for_user(user_id, account_id, api_base_url, location_id)
    ...
    logger.info(f"[BATCH {batch_id}] Initial aggregation: {len(all_data)} credit transactions from {len(user_ids)} users (account: {account_id})")
    ...
    if final_failed_count > 0:
        ...
        raise Exception(f"Credit transactions batch {batch_id} failed: {final_failed_count} user pages could not be processed after retry")
